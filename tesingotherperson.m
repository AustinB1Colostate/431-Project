...
annotations = readcell("anno_flut_418.txt",'ExpectedNumVariables',8);
secondcoldata = val(1,:);
gain = 200;
...
for eventType = eventTypes
    logicalMatch = matches(annotations(:,7), eventType);
    eventInds = find(logicalMatch);
    for i = 1:length(eventInds)
        eventInd = eventInds(i);
        sampleInd = annotations{eventInd,2};
        sampleInd = sampleInd-255364; % Adjust based on the actual start time in your annotations
    
        beginInd = round(sampleInd) + round(2*fs);
        endInd = round(sampleInd) +round(4*fs); % Two second window
        ...
        if beginInd >= 1 && beginInd < endInd - 3
            ...
            [pks, locs] = findpeaks(segment_x, 'MinPeakProminence', 0.2, 'MinPeakDistance', 40);
            figure;
            findpeaks(segment_x, 'MinPeakProminence', 0.2, 'MinPeakDistance', 40);
